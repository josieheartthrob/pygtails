...
The following table is almost directly taken from the pygame documentation.
...
A Quick Primer on Flags
    ...
    for each value in the flag, if the value is "active" it's it's relative
    "bit" will be represented as a 1. If the value is "inactive" it's relative
    bit is represented as a 0.
...
Bitwise Operations
    ...
    Performing operations on flags is usualy done with bitwise operations.
    Bitwise operations deal with the binary representations of integers.
    This is perfect for flags because flags are defined by their binary
    representations.

    Combining two or more flag values is usually done with bitwise-OR (|).
    The result of a bitwise-OR will include a 1 in every place it appeared in
    any of the combining values. The following are examples of the product of
    bitwise-OR operations on two integers:

        8 (1000) | 1 (0001) = 9 (1001)
        6 (0110) | 3 (0011) = 7 (0111)

    Checking to see if a flag value is active is usually done with bitwise-AND
    (&). The result of a bitwise-AND will only have a 1 where both of the
    combining values are 1. The following are examples of the product of 
    bitwise-AND operations on two integers:
...
Using Bitwise Operations with Flags
    ...
    you might check the value of the key mod against KMOD_LCTRL | KMOD_LALT.
    Assuming `mod` is the flag value for the current key mods being pressed,
    a good way to do check if a specific combination is being pressed would
    look something like.

        combo = KMOD_LCTRL | KMOD_LALT
        mod & combo == combo

    This will perform an inclusive check (other key mods can be pressed as well)
...
Key Mod Flags
    ...
    Descriptions left blank are Key Mod Flags that are unclear, and I haven't
    been able to determine what they do.

    Key Mod descriptions prefaced with "Both" shouldn't be confused with
    "either"
...
Display Mode Flags
    ...
    The following table is taken almost directly from the pygame documentation.
